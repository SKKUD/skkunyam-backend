...
import com.skkudteam3.skkusirenorder.src.dto.OrderDenyReqDTO;
...
public class DenyOrderTest {
    ...
    public void payOrderTest_default() throws Exception{
        //given
        OrderDenyReqDTO orderDenyReqDTO = new OrderDenyReqDTO(1L, "재고소진");
        //when
        orderService.denyOrder(orderDenyReqDTO);
        Order order = orderRepository.findById(1L).get();

        //then
        Assertions.assertEquals(order.getOrderStatus(), OrderStatus.CANCEL);
        Assertions.assertEquals(order.getOrderCancellationMessage(), orderDenyReqDTO.getOrderCancellationMessage());
    }
